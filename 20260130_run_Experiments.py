...
def setup_and_run_jobs(args: argparse.Namespace, wd: Path, datadir: Path, 
    ...
    if not args.skip_diluted:
        ...
        for i in (datadir / "diluted_dataset").iterdir():
            if i.is_dir():
                ...
                # check principal existence of the data directories
                assert (i / "primary_sequences").exists(), \
                    f"Data directory {i / 'primary_sequences'} does not exist"
                assert (i / "control_sequences").exists(), \
                    f"Control data directory {i / 'control_sequences'} does not exist"
    ...
    if not args.skip_hybrid:
        ...
        for rep in (datadir / "hybrid_dataset").iterdir():
            if rep.is_dir():
                ...
                assert (rep / "primary_sequences").exists(), \
                    f"Data directory {rep / 'primary_sequences'} does not exist"
                assert (rep / "control_sequences").exists(), \
                    f"Control data directory {rep / 'control_sequences'} does not exist"
    ...
    if not args.skip_simulated:
        ...
        for orderdir in (datadir / "simulated_dataset").iterdir():
            if not orderdir.is_dir():
                continue
            try:
            ...
            assert (orderdir / "wgEncodeAwgTfbsSydhK562Gata1UcdUniPk.narrowPeak").exists(), \
                f"Data directory {orderdir / 'wgEncodeAwgTfbsSydhK562Gata1UcdUniPk.narrowPeak'} does not exist"
            simdir = orderdir / "wgEncodeAwgTfbsSydhK562Gata1UcdUniPk.narrowPeak"
            
            for mfreq in simdir.iterdir():
                if mfreq.is_dir():
                    try:
                        float(mfreq.name)
                    except ValueError:
