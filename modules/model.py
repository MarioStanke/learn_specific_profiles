...
class EpochHistory:
    ...
    def update(self, S: tf.Tensor, loss: float, grad: np.ndarray):
...
class ProfileTracking:
    ...
    def addEpoch(self, epoch: int, P: np.ndarray, P_logit: np.ndarray, grad: np.ndarray, 
        ...
        """ Add a profile to the tracking. Required shapes:
            P: (k, alphabet_size, U'), P_logit: (k, alphabet_size, U'), grad: (n_batches, k, alphabet_size, U'), 
            max_scores: (U',), mean_losses: (U',), masked_sites: (n, 6), 
    ...
    def train(self, verbose=True, verbose_freq=100):
        ...
        while run:
            ...
            for X, _ in ds_train: # shape: (batchsize, ntiles, N, f, tile_size, alphabet_size)
                ...
                S, loss, grad = self.train_step(X)
                # logging.debug(f"[model.train] >>> {S.shape=}, {loss.shape=}, {grad.shape=}")
                epochHist.update(S, loss.numpy(), grad.numpy())
